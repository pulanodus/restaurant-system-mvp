...
-- Create helper function to cache auth.uid() result
-- This avoids repeated JWT parsing which can be expensive on high-traffic queries
CREATE OR REPLACE FUNCTION auth.uid_cached()
RETURNS uuid AS $$
...
-- Update profiles table policies to use auth.uid_cached() instead of auth.uid()
...
-- Create optimized policies using auth.uid_cached()
CREATE POLICY "Users can view their own profile" ON profiles
    FOR SELECT USING (auth.uid_cached() = id);

CREATE POLICY "Users can update their own profile" ON profiles
    FOR UPDATE USING (auth.uid_cached() = id);
...
-- they should use auth.uid_cached() instead for better performance
...
BEGIN
    ...
    ELSE
        RAISE NOTICE '✅ Profiles table: RLS policies optimized with auth.uid_cached()';
    END IF;
