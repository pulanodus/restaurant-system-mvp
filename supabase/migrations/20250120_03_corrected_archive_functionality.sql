-- Archive functionality migration (CORRECTED VERSION)
...
-- Add archive_reason column to sessions table
ALTER TABLE sessions ADD COLUMN IF NOT EXISTS archive_reason TEXT;

-- Add archived_at column to orders table
ALTER TABLE orders ADD COLUMN IF NOT EXISTS archived_at TIMESTAMPTZ;

-- Add archive_reason column to orders table
ALTER TABLE orders ADD COLUMN IF NOT EXISTS archive_reason TEXT;

-- Add archived_at column to cart_items table
ALTER TABLE cart_items ADD COLUMN IF NOT EXISTS archived_at TIMESTAMPTZ;

-- Add archive_reason column to cart_items table
ALTER TABLE cart_items ADD COLUMN IF NOT EXISTS archive_reason TEXT;

-- Create archive tables for old data (without duplicate columns)
