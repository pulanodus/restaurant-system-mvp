...
export default function DynamicAdminLayout({ children }: DynamicAdminLayoutProps) {
    ...
    try {
      ...
      if (session?.user) {
        ...
        if (profile?.role !== 'admin') {
          console.warn('⚠️  User does not have admin role:', profile?.role || 'no role set');
  ...
  const navigation = [
    ...
    { name: 'Audit Logs', href: '/admin/audit', icon: Shield, current: pathname === '/admin/audit' },
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-yellow-100 border-b border-yellow-200 px-4 py-2">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <button 
            onClick={() => {
              const banner = document.querySelector('.bg-yellow-100') as HTMLElement;
