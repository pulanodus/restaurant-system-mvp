...
export default function MenuItem({ item, sessionId }: MenuItemProps): React.JSX.Element {
  ...
  const cartItem = state.items.find((cartItem: any) => cartItem.menu_item_id === item.id);
  const cartItemId = cartItem?.id;

  // Simplified - the centered modal handles all cart operations internally
  const handleModalClose = (): void => {
    setIsModalOpen(false);
  };

  // Handle direct add to cart (for the + button)
  ...
  // Handle decrement (for the - button)
      ...
      >
        ...
        <div className="flex items-center space-x-3 flex-grow min-w-0 md:flex-col md:space-x-0 md:space-y-3 md:w-full">
          ...
          <div className="flex-grow min-w-0 md:w-full">
            ...
            <p className="text-xs text-gray-600 mt-1 line-clamp-2 md:text-sm md:mt-2 italic">{item.description}</p>
