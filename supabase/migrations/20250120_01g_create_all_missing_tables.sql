...
-- This migration creates all tables that are referenced in the multi-tenant migration
...
DROP TRIGGER IF EXISTS trigger_update_split_bills_updated_at ON split_bills;
...
DROP POLICY IF EXISTS "Users can view split bills" ON split_bills;
DROP POLICY IF EXISTS "Users can insert split bills" ON split_bills;
DROP POLICY IF EXISTS "Users can update split bills" ON split_bills;
DROP POLICY IF EXISTS "Users can delete split bills" ON split_bills;
...
-- Create cart_items table if it doesn't exist (in case it wasn't created)
...
DROP POLICY IF EXISTS "Users can view cart items for their session" ON cart_items;
